...
const SidebarHeader = () => (
  <Grid>
    ...
    <Box>
      <Text>
        Pathfinding Algorithms look to find the shortest path between any two
        points. Our goal with this visualizer is to showcase how many
        pathfinding algorithms accomplish this task. Try it for yourself!
      </Text>
    </Box>
  </Grid>
);
...
const SidebarFooter = (props) => (
  <Nav gap="small" responsive>
    <Button
      disabled={props.visualizeButtonDisabled}
      label="Visualize"
      color={"status-ok"}
    ...
    />
    <Button
      disabled={props.resetBoardButtonDisabled}
      label="Reset Board"
      color={"status-error"}
  ...
  return (
    <Box direction="row" height={{ min: "100%" }}>
      <Sidebar
        ...
        footer={
          <SidebarFooter
            onResetBoardClick={props.onResetBoardClick}
            resetBoardButtonDisabled={props.resetBoardButtonDisabled}
            onVisualizeClick={props.onVisualizeClick}
            visualizeButtonDisabled={props.visualizeButtonDisabled}
          />
        }
        pad={{ left: "medium", right: "large", vertical: "medium" }}
        round={[
          { size: "small", corner: "top" },
          { size: "small", corner: "bottom" },
        ]}
      >
        <MainNavigation
          changeAlgorithm={props.changeAlgorithm}
